...
function fileDialogBefore(file,swfObject){
	...
	if(id==""||id==null){
		otherswfu1.customSettings.isUpload=false;
		alert("请先保存,再上传!");
		otherswfu1.eventQueue=[];
	}else{
		otherswfu1.customSettings.isUpload=true;
		otherswfu1.setPostParams({"id":id,"taskId":taskId});
		otherswfu1.startUpload();
	}
}
